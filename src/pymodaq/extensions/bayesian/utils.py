...
AcquisitionKind = {
    'Upper Confidence Bound':  { 'cls' : acquisition.UpperConfidenceBound, 'param_names' : ['kappa', 'exploration_decay', 'exploration_decay_delay']} ,
    'Expected Improvement':   { 'cls' : acquisition.ExpectedImprovement, 'param_names' : ['xi', 'exploration_decay', 'exploration_decay_delay']},
    'Probability of Improvement': { 'cls' : acquisition.ProbabilityOfImprovement, 'param_names' : ['kappa', 'exploration_decay', 'exploration_decay_delay']}
}



AcquisitionParameters = namedtuple('AcquisitionParameters',
                               ['kind', 'kappa', 'xi', 'exploration_decay', 'exploration_decay_delay'])
...
class BayesianAlgorithm:
    ...
    def __init__(self, ini_random: int, bounds: dict, **kwargs):
        ...
        self._acquisition = acquisition.UpperConfidenceBound(kappa=self.kappa)

    def set_acquisition_function(self, kind: str, **kwargs):
        if kind in AcquisitionKind:
            self._acquisition = AcquisitionKind[kind]['cls'](**kwargs)
    ...
    def update_acquisition_function(self):
        ...
        self.kappa = self._acquisition.kappa
    ...
    def ask(self) -> np.ndarray:
        if self.ini_random_points > 0:
           ...
           # self._next_point = self._algo.space.params_to_array(self._algo.suggest())
            self._next_point = self._algo.space.params_to_array(self._acquisition.suggest(self._algo._gp, self.space))
